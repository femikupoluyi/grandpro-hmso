...
export class EvaluationService {
   ...
   */
  static async evaluateApplication(applicationId: string): Promise<EvaluationResult> {
   ...
   */
  private static async evaluateInfrastructure(application: any): Promise<number> {
    let score = 0;
    const bedCapacity = application.proposedBedCapacity || 0;

    // Bed capacity scoring
    if (bedCapacity >= 100) score += 8;
    else if (bedCapacity >= 50) score += 6;
    else if (bedCapacity >= 20) score += 4;
    else if (bedCapacity >= 10) score += 2;

    // Specializations scoring
    const specializations = application.proposedSpecializations || [];
    if (specializations.length >= 5) score += 6;
    else if (specializations.length >= 3) score += 4;
    else if (specializations.length >= 1) score += 2;

    // Document verification scoring
    const hasInfrastructureDocs = application.documents.some(
      (doc: any) => doc.documentType === 'FACILITY_PLAN' || doc.documentType === 'BUILDING_PERMIT'
    );
    if (hasInfrastructureDocs) score += 6;
   ...
   */
  private static async evaluateStaffing(application: any): Promise<number> {
    let score = 0;
    ...
    const hasMedicalLicense = application.documents.some(
      (doc: any) => doc.documentType === 'MEDICAL_LICENSE' || doc.documentType === 'STAFF_CREDENTIALS'
    );
    if (hasMedicalLicense) score += 10;
   ...
   */
  private static async evaluateEquipment(application: any): Promise<number> {
    let score = 0;

    // Check for equipment list in documents
    const hasEquipmentDocs = application.documents.some(
      (doc: any) => doc.documentType === 'EQUIPMENT_LIST' || doc.documentType === 'INVENTORY'
    );
    if (hasEquipmentDocs) score += 10;
    ...
    if (application.proposedBedCapacity >= 50) score += 5;
   ...
   */
  private static async evaluateLocation(application: any): Promise<number> {
    let score = 0;
    const location = application.proposedLocation as any;
   ...
   */
  private static calculateTotalScore(criteria: EvaluationCriteria): number {
   ...
   */
  private static getRecommendation(totalScore: number): 'APPROVE' | 'REJECT' | 'PENDING_REVIEW' {
   ...
   */
  private static generateEvaluationNotes(criteria: EvaluationCriteria, application: any): string[] {
    const notes: string[] = [];
