...
// Validation schemas
const hospitalApplicationSchema = z.object({
  hospitalName: z.string().min(2).max(200),
  contactPerson: z.string().min(2).max(100),
  contactEmail: z.string().email(),
  contactPhone: z.string().regex(/^(\+234|0)[789]\d{9}$/, 'Invalid Nigerian phone number'),
  proposedLocation: z.object({
    state: z.string(),
    localGovernment: z.string(),
    city: z.string(),
    address: z.string(),
  }),
  proposedBedCapacity: z.number().min(5).optional(),
  proposedSpecializations: z.array(z.string()).optional(),
...
const evaluationScoreSchema = z.object({
  ...
  evaluationCriteria: z.object({
    infrastructure: z.number().min(0).max(20),
    staffing: z.number().min(0).max(20),
    equipment: z.number().min(0).max(20),
    location: z.number().min(0).max(20),
    financialViability: z.number().min(0).max(20),
  }),
...
const contractGenerationSchema = z.object({
  ...
  contractDuration: z.number().min(1).max(10), // years
...
export const updateApplicationStatus = async (req: Request, res: Response) => {
  try {
    ...
    if (status === 'UNDER_REVIEW') {
      ...
      updateData.decisionBy = (req as any).user?.id; // Assuming user is attached to request
    }
...
export const evaluateApplication = async (req: Request, res: Response) => {
  try {
    ...
    let status: 'APPROVED' | 'REJECTED' | 'DOCUMENTS_REQUESTED';
...
export const signContract = async (req: Request, res: Response) => {
  try {
    ...
    if (
      (isHospitalSign && contract.signedByGrandPro) ||
